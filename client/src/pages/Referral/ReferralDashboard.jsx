...
export default function ReferralDashboard() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      console.error('Failed to fetch referral dashboard data:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading && !data) {
    return (
      <div className="modern-loading-container">
        ...
        <p className="modern-loading-text">ANALYZING REFERRAL DATA...</p>
  ...
  const { top_referrers = [] } = reports;

  return (
    <div className="modern-dashboard">
      <div className="modern-header-row">
        <div className="modern-header-content">
          <div className="modern-badge-indigo">REFERRAL INTELLIGENCE</div>
          <h1 className="modern-main-title">Referral Commission (Incoming Cost)</h1>
          <p className="modern-subtitle">Tracking patient referrals, total commission costs, and pending payouts</p>
        </div>
      ...
      <div className="modern-metrics-grid">
        <MetricCard title="Total Referrals" value={metrics.total_referrals} icon={<Users />} color="indigo" highlight />
        <MetricCard title="Total Commission Cost" value={metrics.total_commission_cost} icon={<DollarSign />} color="rose" currency="MMK" />
        <MetricCard title="Pending Commission" value={metrics.pending_commission} icon={<AlertCircle />} color="orange" currency="MMK" />
      </div>
      ...
      <div className="modern-charts-row">
        <div className="modern-card chart-main">
          <div className="modern-card-header">
            <div>
              <h3 className="modern-card-title">Top Referrers</h3>
              <p className="modern-card-subtitle">Performance breakdown by referring partner</p>
            </div>
          </div>
              ...
              <table className="modern-table">
                <thead>
                  <tr>
                    ...
                    <th>Total Commission (MMK)</th>
                    <th>Contribution</th>
                  </tr>
                </thead>
                <tbody>
                  {top_referrers.map((ref, idx) => {
                    const contribution = metrics.total_commission_cost > 0 
                      ? (ref.total_commission / metrics.total_commission_cost * 100) 
                      : 0;
                    return (
                      <tr key={idx}>
                        ...
                        <td className="text-gray-600">{ref.organization || 'N/A'}</td>
                        <td className="font-bold">{ref.total_referrals}</td>
                        <td className="text-rose-600 font-bold">{Math.round(ref.total_commission).toLocaleString()}</td>
                        <td>
